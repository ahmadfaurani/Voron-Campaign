#!/usr/bin/env python3
"""
Sentiment Analysis for Extracted Entities - 2026-07-11T180922+08 Extraction Cycle
...
Entity source:  2026-07-11T180922+08_entities_extracted.json (161 entities, 23 successful sources)
Collection:     2026-07-11T180922+08_political_collection_25sources_OPERATIONAL.json
...
ENTITIES_FILE = os.path.join(ENTITIES_DIR, "2026-07-11T180922+08_entities_extracted.json")
COLLECTION_PREFIX = "2026-07-11T180922+08"
...
def split_sentences(text):
    """Sentence splitter that handles common Malay/English punctuation."""
    if not text:
        return []
    # Normalize whitespace and split on sentence-ending punctuation
    text = re.sub(r'\s+', ' ', text)
    # Keep the delimiter by splitting then re-attaching
...
def analyze_entity_sentiment(entity, articles, analyzer):
    ...
    for source_name, article in articles.items():
        ...
        for text in text_content:
            if not text or el not in text.lower():
                continue
            sentences = split_sentences(text)
            for sentence in sentences:
                if el in sentence.lower() and len(sentence.strip()) > 10:
    ...
    # Fallback: no context found in articles
    if not entity_scores:
        scores = analyzer.polarity_scores(entity)
        entity_scores.append(scores['compound'] * 0.5)  # dampen
        contexts.append({
            'source': 'fallback',
...
def calculate_z_scores(sentiment_data):
    ...
    if std_dev == 0:
        return {k: 0 for k in sentiment_data}
    return {entity: (data['mean_score'] - mean) / std_dev for entity, data in sentiment_data.items()}
...
def aggregate_figures_by_party(entity_sentiments):
    ...
    for entity, data in entity_sentiments.items():
        if entity in FIGURE_AFFILIATIONS:
            ...
            figure_data[entity] = {**data, 'party_affiliation': party}
...
"""
    if report['anomalies']['details']:
        ...
        for entity, data in report['anomalies']['details'].items():
            category = entity_categories.get(entity, 'FIGURE')
    ...
    for entity_type, entities in report['entity_sentiments'].items():
        md += f"### {entity_type}\n\n"
        md += "| Entity | Sentiment | Label | Raw Score | Mentions | Z-Score | Anomaly |\n"
        md += "|--------|-----------|-------|-----------|----------|---------|---------|\n"
        for entity, data in sorted(entities.items(), key=lambda x: x[1]['sentiment_score'], reverse=True):
            ...
            md += f"| {entity} | {data['sentiment_score']} | {data.get('sentiment_label','')} | {data['raw_compound']:.4f} | {data['mention_count']} | {data['z_score']:.4f} | {anomaly_mark} |\n"
        md += "\n"
...
def main():
    print("=" * 60)
    print("SENTIMENT ANALYSIS - Entity Extraction Cycle 2026-07-11T180922+08")
    print("=" * 60)
    ...
    anomalies = {"high_positive": [], "high_negative": [], "details": {}}
    ...
    with open(latest_md, 'w') as f:
        f.write(md_report)
    print(f"Latest snapshot saved to: {latest_json} and {latest_md}")
